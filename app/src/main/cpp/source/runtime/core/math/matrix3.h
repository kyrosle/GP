...
*/
  ...
  public:
    ...
    */
    ...
    Matrix3x3(float entry00, float entry01, float entry02, float entry10,
              float entry11, float entry12, float entry20, float entry21,
              float entry22) {
      m_mat[0][0] = entry00;
      m_mat[0][1] = entry01;
      m_mat[0][2] = entry02;
    ...
    float *operator[](size_t row_index) const {
      return (float *) m_mat[row_index];
    }

    Vector3 getColumn(size_t col_index) const {
      assert(0 <= col_index && col_index < 3);
      return Vector3(m_mat[0][col_index], m_mat[1][col_index],
                     m_mat[2][col_index]);
    }
    ...
    Matrix3x3 operator+(const Matrix3x3 &rhs) const {
      ...
      for (size_t row_index = 0; row_index < 3; row_index++) {
        for (size_t col_index = 0; col_index < 3; col_index++) {
          sum.m_mat[row_index][col_index] =
            m_mat[row_index][col_index] + rhs.m_mat[row_index][col_index];
        }
      }
      return sum;
    }
    ...
    Matrix3x3 operator-(const Matrix3x3 &rhs) const {
      ...
      for (size_t row_index = 0; row_index < 3; row_index++) {
        for (size_t col_index = 0; col_index < 3; col_index++) {
          diff.m_mat[row_index][col_index] =
            m_mat[row_index][col_index] - rhs.m_mat[row_index][col_index];
        }
      }
      return diff;
    ...
    Matrix3x3 operator*(const Matrix3x3 &rhs) const {
      ...
      for (size_t row_index = 0; row_index < 3; row_index++) {
        for (size_t col_index = 0; col_index < 3; col_index++) {
          prod.m_mat[row_index][col_index] =
            m_mat[row_index][0] * rhs.m_mat[0][col_index] +
    ...
    friend Vector3 operator*(const Vector3 &point, const Matrix3x3 &rhs) {
      Vector3 prod;
      for (size_t row_index = 0; row_index < 3; row_index++) {
        prod[row_index] = point.x * rhs.m_mat[0][row_index] +
                          point.y * rhs.m_mat[1][row_index] +
    ...
    float determinant() const {
      ...
      float det = m_mat[0][0] * cofactor00 + m_mat[0][1] * cofactor10 +
                  m_mat[0][2] * cofactor20;

      return det;
    }

    void calculateQDUDecomposition(Matrix3x3 &out_Q, Vector3 &out_D,
                                   Vector3 &out_U) const;
