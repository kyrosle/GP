...
namespace Gp {
  ...
  public:
    ...
    Quaternion(const Vector3 &xaxis, const Vector3 &yaxis,
               const Vector3 &zaxis) {
    ...
    static Quaternion getQuaternionFromAngleAxis(const Radian &angle,
                                                 const Vector3 &axis);

    void fromDirection(const Vector3 &direction, const Vector3 &up_direction);

    static Quaternion getQuaternionFromDirection(const Vector3 &direction,
                                                 const Vector3 &up_direction);

    void toAngleAxis(Radian &angle, Vector3 &axis) const;

    /** Constructs the quaternion using 3 axes, the axes are assumed to be
       orthonormal
        @See FromAxes
    */
    void fromAxes(const Vector3 &x_axis, const Vector3 &y_axis,
    ...
    */
    ...
    friend Quaternion operator*(float scalar, const Quaternion &rhs) {
      return Quaternion(scalar * rhs.w, scalar * rhs.x, scalar * rhs.y,
                        scalar * rhs.z);
    }
    ...
    Quaternion inverse() const // apply to non-zero quaternion
    {
      ...
      if (norm > 0.0) {
        ...
        return Quaternion(w * inv_norm, -x * inv_norm, -y * inv_norm,
                          -z * inv_norm);
      } else {
    ...
    */
    ...
    /** Performs Spherical linear interpolation between two quaternions, and
       returns the result. sLerp ( 0.0f, A, B ) = A sLerp ( 1.0f, A, B ) = B
    ...
    */
    static Quaternion sLerp(float t, const Quaternion &kp, const Quaternion &kq,
                            bool shortest_path = false);

    /** Performs Normalised linear interpolation between two quaternions, and
       returns the result. nLerp ( 0.0f, A, B ) = A nLerp ( 1.0f, A, B ) = B
            @remarks
            ...
            being torque-minimal (unless shortestPath=false). However, it's
       performing the interpolation at non-constant velocity; sometimes this is
       desired, sometimes it is not. Having a non-constant velocity can produce a
       more natural rotation feeling without the need of tweaking the weights;
       however if your scene relies on the timing of the rotation or assumes it
       will point at a specific angle at a specific weight value, sLerp is a
       better choice.
        */
    static Quaternion nLerp(float t, const Quaternion &kp, const Quaternion &kq,
                            bool shortest_path = false);
