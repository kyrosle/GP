...
#define CHECK_ERROR(e) case e: acout << "GL Error: "#e << std::endl; break;

namespace Gp {
  bool RenderUtility::CheckAndLogGlError(bool alwaysLog) {
    GLenum error = glGetError();
    if (error == GL_NO_ERROR) {
      if (alwaysLog) {
        acout << "No GL error" << std::endl;
      }
      return true;
    } else {
      switch (error) {
        ...
        default:
          acout << "Unknown GL error: " << error << std::endl;
      }
      return false;
    }
  }
  ...
  float *
  RenderUtility::BuildOrthographicMatrix(float *outMatrix, float halfHeight,
  ...
  float *RenderUtility::BuildIdentityMatrix(float *outMatrix) {
    // column 1
    outMatrix[0] = 1.f;
    outMatrix[1] = 0.f;
    outMatrix[2] = 0.f;
